...
function createSupportHtml(markdown) {
  ...
  return [
    ...
    `      <p><button class="button button-primary" type="button" data-extension-details-button data-extension-details-link data-extension-details-url="${extensionDetailsUrl}">打开 MarkNest 扩展详情页</button></p>`,
    '      <p>如果链接未能自动打开，可以进入 <code>chrome://extensions/</code>，找到 MarkNest，进入详情页后开启 <strong>允许访问文件网址</strong>。当前扩展详情页地址：<code data-extension-details-text>chrome://extensions/?id=</code></p>',
    '    </section>',
...
function createSupportScript() {
  return [
    ...
    '    chrome.runtime.sendMessage({ type: "MARKNEST_OPEN_EXTENSION_DETAILS" });',
    '  });',
    '});',
