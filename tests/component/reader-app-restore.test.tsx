...
import type * as WorkspaceAdapter from '@/features/workspace/lib/workspace-adapter';
...
vi.mock('@/features/workspace/lib/workspace-adapter', async (importOriginal) => {
  const actual = await importOriginal<typeof WorkspaceAdapter>();
...
describe('ReaderApp session restore', () => {
  ...
  it('刷新后恢复 file URL 虚拟目录和当前选中文件', async () => {
    const launchGet = vi.fn().mockResolvedValue({
      'marknest-launch:launch-dir-1': {
        type: 'file-directory',
        directoryName: 'docs',
        directoryUrl: 'file:///Users/example/docs/',
        selectedPathSegments: ['sub', 'guide.md'],
        selectedMarkdown: '# Guide\n\n正文',
        entries: [
          {
            name: 'README.md',
    ...
    const launchRemove = vi.fn().mockResolvedValue(undefined);
    const savedSessions: unknown[] = [];
    workspaceMocks.saveReaderSession.mockImplementation(async (session: unknown) => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        const markdownByUrl = new Map([
          ...
          ['file:///Users/example/docs/sub/guide.md', '# Guide\n\n正文']
        ]);
    ...
    const firstRender = render(<ReaderApp />);
    ...
    const savedSession = savedSessions.at(-1);
    firstRender.unmount();
    window.history.replaceState(null, '', '/');
    delete (globalThis as { chrome?: unknown }).chrome;
    workspaceMocks.loadReaderSession.mockResolvedValue(savedSession);
    workspaceMocks.saveReaderSession.mockClear();
    ...
    expect(screen.getByRole('button', { name: /README\.md/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /guide\.md/ })).toHaveAttribute('aria-current', 'page');
    expect(screen.getByText('已恢复本地文件目录阅读状态。')).toBeInTheDocument();
  });
});
